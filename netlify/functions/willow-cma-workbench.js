...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // If no action parameter, serve HTML interface
        if (!action) {
...
<body>
    <div class="container">
        ...
        <div id="cma-workbench" class="tab-content active">
            <div class="section">
                <div class="section-title">📋 Existing CMA Status</div>
            ...
            <div class="section">
                <div class="section-title">🚀 Generate New CMA</div>
                
                <div class="form-group">
                    <label class="form-label">Property Address *</label>
                    <input type="text" id="cmaAddress" class="form-input" placeholder="123 Main St, Huntington Beach, CA 92648">
                </div>
                ...
                <div id="autoOptimizedParams" class="info-card">
                    ...
                    <div class="info-card-content">
                        ...
                        <div class="form-row" style="margin-top: 15px;">
                            <div>
                                <label class="form-label">Search Radius (mi) 🤖</label>
        ...
        <div id="homebeat-manager" class="tab-content">
            <div class="section">
                <div class="section-title">🏠 Active Homebeats</div>
            ...
            <div class="section">
                <div class="section-title">📊 Engagement Summary</div>
        ...
        <div id="action-planner" class="tab-content">
            <div class="section">
                <div class="section-title">⚡ Triggered Actions (Based on Engagement)</div>
            ...
            <div class="section">
                <div class="section-title">📝 Manual Action Creation</div>
                <div class="form-row">
                    <div>
    ...
    <script>
        ...
        async function loadPersonData() {
            try {
                const response = await fetch(\`/.netlify/functions/willow-cma-workbench?action=getPersonData&personId=\${personId}\`);
        ...
        // Display existing CMA
        function displayExistingCMA(data) {
            const container = document.getElementById('existingCMA');
            
            if (data.customWILLOWCMADate && data.customWILLOWCMALink) {
                container.innerHTML = \`
                    <div class="info-card-title">Last CMA Generated</div>
                    <div class="info-card-content">
                        <div><strong>Date:</strong> \${new Date(data.customWILLOWCMADate).toLocaleDateString()}</div>
                        <div><strong>Property:</strong> \${data.customWILLOWCMAAddress || 'N/A'}</div>
                        <div><strong>Estimated Value:</strong> \${data.customWILLOWCenterValue ? '
...
function cloudCMAAPIRequest(method, path, data = null) {
 ...
 + parseInt(data.customWILLOWCenterValue).toLocaleString() : 'N/A'}</div>
        ...
        async function generateCMA() {
            ...
            try {
                ...
                resultDiv.innerHTML = \`
                    <div class="success">
                        ✅ <strong>CMA Generated Successfully!</strong><br>
                        \${data.homebeatCreated ? '✅ Homebeat subscription created' : ''}<br>
                        <div class="btn-group" style="margin-top: 10px;">
                            <button class="btn btn-secondary" onclick="window.open('\${data.cmaUrl}', '_blank')">
                                📄 View CMA
                            </button>
                            \${data.homebeatUrl ? \`<button class="btn btn-secondary" onclick="window.open('\${data.homebeatUrl}', '_blank')">🏠 View Homebeat</button>\` : ''}
                        </div>
                    </div>
                \`;
                ...
                resultDiv.innerHTML = \`<div class="error">❌ Failed to generate CMA: \${error.message}</div>\`;
            }
        }
        ...
        async function loadHomebeatData() {
            try {
                const response = await fetch(\`/.netlify/functions/willow-cma-workbench?action=getHomebeatData&personId=\${personId}\`);
                const data = await response.json();
        ...
        function displayHomebeats(homebeats) {
            ...
            container.innerHTML = homebeats.map(hb => {
                ...
                if (views === 0) {
                    ...
                    engagementBadge = '<span class="engagement-badge badge-critical">🔥 HIGH</span>';
                } else if (views >= 5) {
                    engagementClass = 'medium-engagement';
                    engagementBadge = '<span class="engagement-badge badge-high">⚠️ MEDIUM</span>';
                } else {
                    engagementClass = 'low-engagement';
                    engagementBadge = '<span class="engagement-badge badge-medium">📊 LOW</span>';
                }

                return \`
                    <div class="homebeat-item \${engagementClass}">
                        <div class="homebeat-header">
                            <div class="homebeat-title">🏠 \${hb.property_address || 'Unknown Address'}</div>
                            \${engagementBadge}
                        </div>
                        \${needsResend ? \`
        ...
        async function resendHomebeat(homebeatId) {
            ...
            try {
                ...
                alert('✅ Homebeat resent successfully!');
                await loadHomebeatData();
            ...
            container.innerHTML = actions.map(action => \`
                <div class="action-card \${action.priority}">
                    <div class="action-header">
                        \${action.priority === 'critical' ? '🔥 CRITICAL ACTION RECOMMENDED' : action.priority === 'hot' ? '⚠️ HOT ACTION SUGGESTED' : '📊 WARM ACTION SUGGESTED'}
                    </div>
        ...
        async function createTask(property, action, urgency) {
            ...
            try {
                ...
                alert(\`✅ Task created and assigned to \${partner}!\`);
        ...
        async function createManualAction() {
            ...
            try {
                ...
                resultDiv.innerHTML = \`<div class="success">✅ \${actionType} created successfully and assigned to \${assignTo}!</div>\`;
                document.getElementById('actionNotes').value = '';

            } catch (error) {
                resultDiv.innerHTML = \`<div class="error">❌ Failed to create action: \${error.message}</div>\`;
            }
        }
