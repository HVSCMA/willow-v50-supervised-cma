...
// JWT Secret (must match auth-login.js)
...
async function handleStandaloneMode(leadId, baseHeaders, event) {
  ...
  if (!auth.valid) {
    console.log('❌ STANDALONE MODE: Unauthorized access attempt -', auth.error);
    return {
      statusCode: 401,
      ...
      body: `
        ...
        </head><body style="font-family: system-ui; text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center;">
          <div style="background: white; padding: 40px; border-radius: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.2); max-width: 500px;">
            <h1 style="color: #dc2626; font-size: 48px; margin-bottom: 20px;">🔒</h1>
            <h2 style="color: #1e293b; margin-bottom: 16px;">Authentication Required</h2>
            <p style="color: #64748b; margin-bottom: 24px;">${auth.error}</p>
            <p style="color: #94a3b8; font-size: 14px;">Redirecting to login page...</p>
            <p style="margin-top: 20px;"><a href="/login.html" style="color: #2563eb; text-decoration: none; font-weight: 600;">Click here if not redirected automatically</a></p>
          </div>
        </body></html>
      `
    };
  ...
  console.log('✅ STANDALONE MODE: Authenticated user:', auth.user.name, '(', auth.user.role, ')');
...
exports.handler = async (event, context) => {
  ...
  if (params.leadId) {
    ...
    return await handleStandaloneMode(params.leadId, baseHeaders, event);
  }
